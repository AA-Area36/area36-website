...
export default async function GeneralServiceConferencePage() {
  const locale = await getRequestLocale()
  const conferenceContent = await getContent("generalServiceConference", locale)
  const { t } = createTranslator(conferenceContent)

  return (
    <>
      <PageHeader
        title={t("header.title", "General Service Conference")}
        description={t(
          "header.description",
        ...
        <section className="py-12 sm:py-16" aria-labelledby="background-heading">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <h2 id="background-heading" className="text-2xl font-bold text-foreground mb-4">
              {t("background.title", "Background Material")}
            </h2>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-amber-800 dark:text-amber-200">
                      {t("background.protectedTitle", "Password Protected Materials")}
                    </p>
                    <p className="text-sm text-amber-700 dark:text-amber-300 mt-1">
                      {t(
                        "background.protectedBody",
        ...
        <section className="py-12 sm:py-16 bg-muted/30" aria-labelledby="materials-heading">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <h2 id="materials-heading" className="text-2xl font-bold text-foreground mb-6">
              {t("advisoryActions.title", "Conference Advisory Actions")}
            </h2>
            <p className="text-muted-foreground mb-8">
              {t(
                "advisoryActions.description",
        ...
        <section className="py-12 sm:py-16" aria-labelledby="reports-heading">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <h2 id="reports-heading" className="text-2xl font-bold text-foreground mb-6">
              {t("finalReports.title", "Final Reports")}
            </h2>
            <p className="text-muted-foreground mb-8">
              {t(
