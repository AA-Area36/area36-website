...
describe("quorum check-in accessibility and submission", () => {
  ...
  it("acquires a token before final validation and submits only once", async () => {
    ...
    await waitFor(() => expect(screen.getByRole("heading", { name: "You’re checked in" })).toHaveFocus())
    fireEvent.click(screen.getByRole("button", { name: "Check in another attendee" }))
    await waitFor(() => expect(screen.getByRole("textbox", { name: "First and last name" })).toHaveFocus())
    expect(submitQuorumRegistration).toHaveBeenCalledTimes(1)
    expect(submitQuorumRegistration).toHaveBeenCalledWith(event.eventKey, expect.objectContaining({ recaptchaToken: "synthetic-security-token", newsletterDelivery: "neither" }), expect.stringMatching(/^[A-Za-z0-9_-]{18}$/))
  })
})
