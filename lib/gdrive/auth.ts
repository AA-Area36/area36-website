...
const TOKEN_CACHE_KEY = "https://cache.internal/gdrive-token"

/**
 * Get token from Cloudflare Cache API
 */
async function getTokenFromCache(): Promise<{ accessToken: string; expiresAt: number } | null> {
...
export async function getAccessToken(credentials: GDriveCredentials): Promise<string> {
  ...
  await setTokenInCache(accessToken, expiresAt)
